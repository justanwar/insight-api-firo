...
function BlockController(options) {
    ...
    this.knownHalvings = this.node.network.knownHalvings;
    this.knownHalvingsLen = this.knownHalvings.length;
    this.lastKnownHalving = this.knownHalvings[this.knownHalvingsLen - 1];
}
...
BlockController.prototype.getPoolInfo = function (block) {
    ...
    this.node.log.info("pools.json", "Cannot find pool: " + coinbaseBuffer.toString('hex'));
...
BlockController.prototype.getBlockReward = function (height) {
    ...
    if (height >= this.lastKnownHalving) {
        halvings = this.knownHalvingsLen;
    } else {
        this.knownHalvings.forEach(function (knownHalving) {
            if (height >= knownHalving) {
            halvings += 1;
            }
        });
    }
    halvings += Math.max(0, Math.floor((height - this.lastKnownHalving) / 420000));
