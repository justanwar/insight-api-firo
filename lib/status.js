...
StatusController.prototype.show = function (req, res) {
    ...
    switch (option) {
        ...
        case 'getBestBlockHash':
            ...
            break;
        case 'getMempoolSporks':
          this.getMempoolSporks(function (err, result) {
            if (err) {
              return self.common.handleErrors(err, res);
            }
            res.jsonp(result);
          ...
          break;
        case 'getBlockchainSporks':
          this.getBlockchainSporks(function (err, result) {
            if (err) {
              return self.common.handleErrors(err, res);
            }
            res.jsonp(result);
...
StatusController.prototype.getMempoolSporks = function (callback) {
  this.node.services.bitcoind.spork("list", function (err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {"data": info.result.mempool});
  });
};

StatusController.prototype.getBlockchainSporks = function (callback) {
  this.node.services.bitcoind.spork("list", function (err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {"data": info.result.blockchain});
  });
};
