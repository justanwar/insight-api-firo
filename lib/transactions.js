...
TxController.prototype.transformTransaction = function (transaction, options, callback) {
        ...
        if (transformed.vin && transformed.vin.length) {
            for (var i = 0; i < transformed.vin.length; i++) {
                ...
                if (vin && vin.scriptSig && vin.scriptSig.asm && vin.scriptSig.asm.startsWith('OP_ZEROCOINSPEND ')) {
                    vin.addr = 'Zerospend';
                } else if (vin && vin.scriptSig && vin.scriptSig.asm && vin.scriptSig.asm.startsWith('OP_ZEROCOINSPEND_V3 ')) {
                    vin.addr = 'Sigmaspend';
    ...
    if (transformed.vout && transformed.vout.length) {
        for (var o = 0; o < transformed.vout.length; o++) {
            ...
            } else if (vout && vout.scriptPubKey && vout.scriptPubKey.asm && vout.scriptPubKey.asm.startsWith('OP_ZEROCOINMINT_V3 ')) {
                if (!vout.scriptPubKey.addresses) {
                    vout.scriptPubKey.addresses = [];
                }
